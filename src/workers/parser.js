...
const btceParser = (data) => {
  ...
  return resultTemplate('btc-e.com', json.btc_eur.avg);
};

const blockchainParser = (data) => {
  const json = JSON.parse(data.body);
  return resultTemplate('blockchain.com', json.EUR.last);
};

const coindeskParser = (data) => {
  const json = JSON.parse(data.body);
  return resultTemplate('CoinDesk.com', json.bpi.EUR.rate);
};
